...
local weapon = {}
	...
	weapon.Ammo = 480
	weapon.Delay = 60 / 3400 -- claimed
	weapon.HeatRateUp = 0.57
	weapon.HeatRateDown = 0.2
	
	
	weapon.Attack = function( ent )
			local effectdata = EffectData()
			effectdata:SetOrigin( ent:LocalToWorld( Vector(92,83.8 * (self.FireLeft and 1 or -1),43.84) ) )
			effectdata:SetNormal( ent:GetForward() )
			effectdata:SetEntity( ent )
			util.Effect( "lvs_muzzle", effectdata )
		...
		local bullet = {}
		bullet.Src 	= ( ent:LocalToWorld( Vector(92,83.8 * (self.FireLeft and 1 or -1),43.84) ) )
		bullet.Dir 	= (trace.HitPos - bullet.Src):GetNormalized() --ent:GetForward()
		bullet.Spread 	= Vector( 0,  0.01, 0.01 )
		...
		ent:EmitSound( "lvs_copters/weapons/gsh23_single.wav" )
	...
	weapon.Attack = function( ent )
		...
		local bullet = {}
		bullet.Src 	= ( ent:LocalToWorld( Vector(58.38,111.89 * (self.FireLeft and 1 or -1),38.94) ) )
